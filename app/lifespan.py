...
async def lifespan(app: FastAPI):
    ...
    # Gate is the market-data/scanner venue. Bitget is the execution venue.
    # Keeping separate clients prevents a Bitget market list from silently
    # changing the original Gate universe/ranking strategy.
    gate_rest = GateRestClient(settings)
    gate = GateClient(gate_rest, GateFuturesWebsocket(settings.gate_ws_url))
    bitget_rest = BitgetRestClient(settings)
    bitget = BitgetClient(bitget_rest)
    coinglass = CoinGlassClient(settings)
    notifier = NotificationService(settings, repository)
    trading = TradingService(bitget, repository, settings, notifier)
    ...
    app.state.services = AppState(settings, gate, bitget, repository, notifier, scanner, replay, backtest, trading, scheduler)
    await trading.start()
    if scheduler:
        scheduler.start()
    try:
