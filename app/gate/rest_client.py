...
def _numeric_text(value: Any) -> str:
    """Format numeric query values in the form Gate documents."""
...
class GateRestClient:
    ...
    async def set_leverage(
        ...
        params: dict[str, Any] = {"leverage": _numeric_text(leverage), "margin_mode": margin_mode}
        if dual_side:
    ...
    async def set_position_mode(self, position_mode: str = "single") -> dict[str, Any]:
        ...
        if mode not in {"single", "dual", "dual_plus"}:
            raise ValueError("Gate position mode must be single, dual, or dual_plus")
        payload = await self.request(
            "POST",
            POSITION_MODE,
    ...
    async def set_cross_leverage_legacy(self, contract: str, leverage: float) -> dict[str, Any]:
        ...
        params = {"leverage": "0", "cross_leverage_limit": _numeric_text(leverage)}
        payload = await self.request(
            "POST",
