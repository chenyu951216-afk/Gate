...
class NotificationService:
    ...
    async def send_order(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        reason_map = {
            ...
            "reversal_confirmation_pending": "反向 30m 排名或持倉 15m 論點尚未共同確認，不反手",
            "skipped_exchange_constraint": "Bitget 合約限制無法安全接受本次計算出的下單參數",
            "failed_exchange_preflight": "Bitget 帳戶模式、槓桿或下單前檢查未能確認",
            "rejected_risk": "交易計畫未通過必要風險檢查",
            "failed": "交易所或系統執行失敗",
            "time_decay_grace": "持倉時效到期，但已收 15m 趨勢仍健康或已有進展",
            "position_thesis_watch": "單根已收 15m 出現弱化，只觀察、不平倉",
            "position_thesis_recovered": "15m 弱化未持續，判定為雜訊並恢復正常管理",
            "position_exit_armed": "持倉 15m 多因子論點已失效，開始由 5m 尋找退出點",
            "position_exit_disarmed": "退出前 15m 趨勢恢復，取消 5m 退出狀態",
            "position_exit_executed": "15m 趨勢失效後已由 5m／回本目標完成退出",
            "protection_reconciled": "交易所止損止盈保護已重新核對",
        ...
        for key in (
            ...
            "thesis_failure_kind", "thesis_failure_confirmations",
            "position_thesis_state", "thesis_score", "thesis_evidence",
            "recovery_target", "exit_5m_score", "exit_5m_evidence",
            "exit_5m_confirmations",
            "volatility_regime", "management_atr",
            "stop_source", "planning_atr", "take_profit_allocation_source",
            "time_stop_horizon_hours",
        ):
            if key in action:
