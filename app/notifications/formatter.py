...
def _compact_item(item: dict[str, Any], icon: str) -> str:
    ...
    lifecycle = {
        "NEW": "新進",
        "PERSISTING": f"持續{item.get('consecutive_qualified_scans', 1)}輪",
        "REQUALIFIED": "重新進榜",
        "DIRECTION_FLIP": "方向翻轉",
    }.get(str(item.get("signal_lifecycle", "NEW")), "新進")
    return (
        f"{icon} #{item.get('rank', '—')} {item.get('contract', '—')}｜{direction}｜"
        f"{signal_class}・{lifecycle}｜"
...
def format_scan(result: dict[str, Any]) -> list[str]:
    ...
    combined = rankings.get("combined", [])
    newly_actionable = [
        item
        for item in combined
        if item.get("signal_lifecycle") != "PERSISTING"
    ]
    persisting = [
        item
        for item in combined
    ...
    header = "\n".join(
        [
            ...
            f"股票休市略過 {diagnostics.get('stock_market_closed_count', 0)}｜新進榜即進入下單流程",
    ...
    for content in (
        header
        + "\n\n"
        + _section("🚀 新進／重新進榜／方向翻轉", newly_actionable, "⭐"),
        _section("🔁 持續進榜", persisting, "🔁"),
    ):
