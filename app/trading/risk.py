...
def build_execution_plan(
    ...
    base_multipliers = (
        first_multiple,
        max(2.0, first_multiple + 0.8),
        max(3.0, first_multiple + 1.8),
    )
    base_targets = [
        entry + risk_distance * multiplier if side == "long" else entry - risk_distance * multiplier
        for multiplier in base_multipliers
    ]
    candidates = _target_candidates(metrics, side, entry, risk_distance, atr15)
    targets: list[float] = []
    sources: list[str] = []
    previous_rr = 0.0
...
def managed_stop_candidate(
    *,
    ...
    if side not in {"long", "short"} or price <= 0 or entry <= 0 or risk_distance <= 0 or atr15 <= 0:
        return {
    ...
    break_even_ready = (
        peak_r >= float(getattr(settings, "break_even_activation_r", 1.20))
        or "TP1" in completed
    )
    structure_ready = (
    ...
    if state in {"high_volatility", "extreme"}:
        structure_buffer = 1.20
    elif state in {"range", "low_volatility", "mixed"}:
        structure_buffer = 0.80
    else:
    ...
    if structure_ready:
        ...
        if structure_level is not None:
            structure_stop = (
                structure_level - structure_buffer * atr15
                if side == "long"
                else structure_level + structure_buffer * atr15
            )
    ...
    if runner_ready:
        if state in {"high_volatility", "extreme"}:
            trail_atr = float(getattr(settings, "high_volatility_trailing_atr", 3.5))
        elif state in {"range", "low_volatility", "mixed"}:
            trail_atr = float(getattr(settings, "range_trailing_atr", 2.2))
        else:
            trail_atr = float(getattr(settings, "trend_trailing_atr", 3.0))
        chandelier = (
            favorable_extreme - trail_atr * atr15
            if side == "long"
            else favorable_extreme + trail_atr * atr15
        )
...
def adaptive_operational_leverage(
    ...
    """Keep approximate liquidation distance well beyond the strategy stop."""
