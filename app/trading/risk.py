...
def _liquidity_aware_stop(
    ...
    # Reserve room for the buffer so the final stop remains within the
    # strategy's tradable 4.5 ATR envelope.
    maximum_distance = max(1.0, 4.5 - buffer_atr) * atr15
    prices = [
        price
        for raw in levels
...
def build_execution_plan(
    ...
    for candidate, base in zip(coinglass_targets, base_targets, strict=True):
        ...
        candidate_net_rr = (candidate_distance - fee_distance) / risk_distance if risk_distance > 0 else 0.0
        valid_candidate = (
            candidate is not None
            and candidate_distance > 0
            and candidate_rr + 1e-9 >= minimum_rr
            and candidate_net_rr + 1e-9 >= net_rr
        )
        if valid_candidate:
