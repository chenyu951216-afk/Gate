...
def dynamic_position_notional(
    *,
    ...
    signal_class = str(metrics.get("signal_class", "trend")).lower()
    base = float(settings.risk_per_trade_pct)
    if signal_class == "tactical":
        base *= float(getattr(settings, "tactical_risk_multiplier", 0.85))
    regime = {
        "low_volatility": 1.05,
        "normal": 1.0,
        "bullish": 1.0,
...
def build_execution_plan(
    ...
    signal_class = str(ranking.get("signal_class", "trend")).lower()
    aligned_trend = (side == "long" and state == "bullish") or (
        side == "short" and state == "bearish"
    )
    if signal_class == "tactical":
        stage_two, stage_three = 2.00, 2.80
        tp_percents = (0.40, 0.35, 0.20)
        runner_percent = 0.05
        allocation_source = "30m_tactical_fast_realization"
    elif volatility_regime == "expansion" and aligned_trend:
    ...
    return {
        ...
        "closed_favorable_extreme": entry,
        "favorable_close_extreme": entry,
        "peak_r_multiple": 0.0,
        ...
        "signal_horizon": ranking.get("signal_horizon", "4h_30m_trend"),
...
def managed_stop_candidate(
    *,
    ...
    closed_candle_extreme = _positive(
        context.get("last_high15" if side == "long" else "last_low15")
    ) or entry
    previous_extreme = (
        _positive(plan.get("closed_favorable_extreme"))
        or _positive(plan.get("favorable_extreme"))
        or entry
    )
    favorable_extreme = (
        max(previous_extreme, closed_candle_extreme)
        if side == "long"
        else min(previous_extreme, closed_candle_extreme)
    )
    closed_reference = _positive(context.get("last_close15")) or entry
    previous_close_extreme = (
        _positive(plan.get("favorable_close_extreme")) or entry
    )
    favorable_close_extreme = (
        max(previous_close_extreme, closed_reference)
        if side == "long"
        else min(previous_close_extreme, closed_reference)
    )
    favorable_close_move = (
        favorable_close_extreme - entry
        if side == "long"
        else entry - favorable_close_extreme
    )
    peak_r = max(
        float(plan.get("peak_r_multiple") or 0.0),
        favorable_close_move / risk_distance,
    )
    current_closed_move = (
        closed_reference - entry
        if side == "long"
        else entry - closed_reference
    )
    current_closed_r = current_closed_move / risk_distance
    ...
    thesis_stable = thesis_state not in {"WATCH", "EXIT_ARMED"}
    break_even_ready = thesis_stable and (
        peak_r >= break_even_activation
        or ("TP1" in completed and current_closed_r >= 0.50)
    )
    structure_ready = (
    ...
    phase = "INITIAL_RISK"
    if not thesis_stable:
        return {
            "candidate_stop": current_stop,
            "phase": (
                "EXIT_ARMED_5M"
                if thesis_state == "EXIT_ARMED"
                else "THESIS_WATCH"
            ),
            "favorable_extreme": favorable_extreme,
            "closed_favorable_extreme": favorable_extreme,
            "favorable_close_extreme": favorable_close_extreme,
            "peak_r_multiple": peak_r,
            "trail_source": "held_during_15m_thesis_review",
