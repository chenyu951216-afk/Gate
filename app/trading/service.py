...
class TradingService:
    ...
    async def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "position_mode": self.settings.gate_position_mode,
        }
    ...
    def _limit_entry_price(ticker: dict[str, Any], side: str, mark_price: float, info: Any, offset_pct: float) -> float:
        ...
        if raw_price <= 0:
            raise TradingRiskError("INVALID_LIMIT_PRICE", "Gate limit entry price is invalid")
        return raw_price
    ...
    async def _open_candidate(self, candidate: dict[str, Any], info: Any) -> dict[str, Any]:
        ...
        body = {
            ...
            # Gate can bind an initial stop to the entry order itself. This is
        ...
        if not entry_order_id:
            raise TradingRiskError("ENTRY_ORDER_ID_MISSING", "Gate did not return a limit order id")
        try:
        ...
        if actual_size > 0 and self._position_margin_mode(position) != "cross":
            ...
            raise TradingRiskError(
                ...
                f"Gate filled {info.name} outside cross margin; the position was closed for safety",
            )
        if actual_size <= 0:
    ...
    async def _ensure_single_position_mode(self) -> None:
        """Ensure the order/close payloads match Gate one-way position mode."""
        try:
            positions = await self.gate.rest.get_positions()
        except Exception as exc:
            raise TradingRiskError(
                "POSITION_MODE_NOT_CONFIRMED",
                f"cannot read Gate positions for mode verification: {type(exc).__name__}: {exc}",
            ) from exc
        active_modes = {
        ...
        if active_modes & {"dual", "dual_long", "dual_short", "dual_plus"}:
            raise TradingRiskError(
                "POSITION_MODE_NOT_CONFIRMED",
                "Gate has an existing dual/dual_plus position; close or convert it manually before new orders",
            )
        # Gate refuses the global mode endpoint while *any* position is held.
        ...
        try:
            ...
            raise TradingRiskError(
                "POSITION_MODE_NOT_CONFIRMED",
                f"cannot read Gate position mode: {type(exc).__name__}: {exc}",
            ) from exc
        ...
        if account.get("in_dual_mode") is True or raw_mode in {"dual", "dual_plus"} or not raw_mode:
            try:
                ...
                raise TradingRiskError(
                    "POSITION_MODE_NOT_CONFIRMED",
                    f"Gate could not switch to single position mode: {type(exc).__name__}: {exc}",
                ) from exc
            ...
            if response.get("in_dual_mode") is True or response_mode in {"dual", "dual_plus"}:
                raise TradingRiskError(
                    "POSITION_MODE_NOT_CONFIRMED",
                    "Gate still reports dual position mode after requesting single mode",
                )
            ...
            try:
                ...
                raise TradingRiskError(
                    "POSITION_MODE_NOT_CONFIRMED",
                    f"cannot verify Gate single position mode: {type(exc).__name__}: {exc}",
                ) from exc
            if verified.get("in_dual_mode") is not False and str(verified.get("position_mode") or "").lower() != "single":
                raise TradingRiskError(
                    "POSITION_MODE_NOT_CONFIRMED",
                    "Gate did not confirm single position mode",
                )
            return
        raise TradingRiskError(
            "POSITION_MODE_NOT_CONFIRMED",
            f"unsupported Gate position mode: {raw_mode or 'unknown'}",
        )

    async def _ensure_cross_margin(self, contract: str, leverage: float) -> None:
        """Switch and verify Gate cross margin before an entry is submitted.

        Gate exposes both a dedicated margin-mode endpoint and a mode-aware
        leverage endpoint. The legacy cross-leverage endpoint is retained as a
        fallback because Gate documents it as the explicit cross-mode form.
        """
        errors: list[str] = []
        ...
        # Prefer the position returned by Gate after the mode/leverage calls.
        # This is the only confirmation that describes the state which the
        # following entry order will actually inherit.  Some Gate clusters
        ...
        # with no position row yet), accept two independent successful Gate
        ...
        detail = "; ".join(errors) or "Gate returned no cross-margin confirmation"
    ...
    async def _create_trigger(
        ...
        if is_stop:
            ...
            # Gate uses `amount` for decimal contract sizes in price orders;
            # `size` is an integer field for this endpoint.
            initial["amount"] = size
        else:
            ...
            # same string here can make Gate reject an otherwise valid TP.
        ...
        trigger: dict[str, Any] = {
            "strategy_type": 0,
            # Gate's own price-order example uses the latest trade price and
            # its rule validation compares the trigger against last_price.
            # Using mark price here can make a valid stop/TP fail when mark and
            # last differ by a tick, especially on fast-moving contracts.
            "price_type": 0,
        ...
        response = await self.gate.rest.create_price_order(
            {
                ...
                "pos_margin_mode": plan.get("margin_mode", self.settings.gate_margin_mode),
            }
        )
    ...
    async def _verify_exchange_protection(self, plan: dict[str, Any]) -> None:
        ...
        raise TradingRiskError(
            ...
            f"Gate did not confirm exchange protection orders for {plan['contract']}: {','.join(missing)}",
        )
    ...
    def _plan_from_position(
        ...
        plan["margin_mode"] = self._position_margin_mode(position) or self.settings.gate_margin_mode
        return plan
    ...
    def _notional(self, contract: str) -> float:
        ...
        if upper in self._market_driver_notional_contracts():
