...
class TradingService:
    ...
    async def _ensure_cross_margin(self, contract: str, leverage: float) -> None:
        ...
        if mode_response and self._response_confirms_cross(mode_response) and leverage_response is not None and leverage_mode != "isolated":
            return
        mode_mode = self._position_margin_mode(mode_response)
        if self._response_confirms_cross(leverage_response) and leverage_mode != "isolated" and mode_mode in (None, "cross"):
            return
        ...
        try:
            ...
            if self._response_confirms_cross(current) and mode_response is not None and leverage_response is not None:
                return
            if current:
