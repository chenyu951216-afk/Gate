...
class TradingService:
    ...
    def __init__(self, execution_client: Any, repository: Any, settings: Any, notifier: Any | None = None):
        ...
        # This is populated at submission time with the signal's 15m ATR.  It
        # lets volatile contracts receive a wider, still capped, guard while
        # keeping a safe global fallback after a process restart.
    ...
    async def _apply_scan_protection_update(
        ...
        take_profit_threshold = max(
            float(getattr(self.settings, "scan_tp_min_move_atr", 0.20)) * atr15,
            entry * 0.001,
        )
        changed: list[str] = []
        errors: list[str] = []
        opened_at = self._as_utc_datetime(plan.get("opened_at"))
        position_age_seconds = (
            max(0.0, (_now() - opened_at).total_seconds())
            if opened_at is not None
            else 0.0
        )
        initial_stop_locked = position_age_seconds < float(
            self.settings.initial_stop_lock_seconds
        )
        stop_profit_ready = current_r >= float(self.settings.scan_stop_min_profit_r)
        ...
        if (
            stop_is_live_side
            and not initial_stop_locked
            and stop_profit_ready
            ...
            try:
                new_id = await self._replace_trigger(
                    plan, "stop", proposed_stop, str(size), record["position_key"], old_id
                )
        ...
        last_scan_tp_update = plan.get("last_scan_tp_update")
        scan_tp_locked = False
        if last_scan_tp_update:
            try:
                scan_tp_locked = (
                    _now() - datetime.fromisoformat(last_scan_tp_update)
                ).total_seconds() < float(
                    getattr(self.settings, "scan_tp_update_cooldown_seconds", 1800)
                )
            except ValueError:
                pass
        min_profit_r = max(
            float(getattr(self.settings, "scan_tp_min_profit_r", 1.0)),
            float(getattr(self.settings, "minimum_order_rr", 1.0)),
        )
        ...
        for target in plan.get("take_profits", []):
            ...
            moving_closer_for_profit = current_r >= min_profit_r and (
                new_price < old_price if side == "long" else new_price > old_price
            )
            # A scan may only tighten an unfinished TP after the position is
            # already at least 1R in profit.  In particular, never replace
            # 1660 with a newly calculated 1680 while price is 1650: that
            # chases an unfilled target and can oscillate on every scan.
            # Further TP extensions belong exclusively to the profit-runner
            # manager, which has its own distance and cooldown guards.
            new_rr = abs(new_price - entry) / current_risk if current_risk > 0 else 0.0
            if scan_tp_locked or not new_is_live_side or not moving_closer_for_profit or new_rr + 1e-9 < min_profit_r:
                continue
            ...
            try:
                ...
                plan["last_scan_tp_update"] = _now().isoformat()
                # The manager's extension and weakness guards share this
                # timestamp so a scan-time replacement cannot immediately be
                # overwritten by a second TP policy in the same cycle.
                plan["last_take_profit_update"] = plan["last_scan_tp_update"]
                scan_tp_locked = True
        ...
        return {
            ...
            "errors": errors,
            "position_age_seconds": round(position_age_seconds, 1),
            "initial_stop_locked": initial_stop_locked,
        }
    ...
    async def _open_candidate(self, candidate: dict[str, Any], info: Any) -> dict[str, Any]:
        ...
        atr_pct = _number(plan.get("atr15")) / limit_price if limit_price > 0 else 0.0
    ...
    async def _verify_exchange_protection(self, plan: dict[str, Any]) -> None:
        ...
        for target in plan.get("take_profits", []):
            ...
            if stage not in plan.get("completed_stages", []) and ids.get(stage) not in (None, ""):
                expected[stage] = (
                    str(ids[stage]),
                    _number(target.get("price")),
                    partial_close_size(plan["side"], str(plan.get("entry_size") or 0), target["percent"]),
                )
        if not expected:
    ...
    def _time_stop_horizon_hours(plan: dict[str, Any], settings: Any | None) -> float:
        ...
        if state in {"bullish", "bearish"}:
            return values["trend"]
        if state in {"range", "low_volatility"}:
            return values["range"]
        if state in {"extreme", "high_volatility"}:
