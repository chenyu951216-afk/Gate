...
class TradingService:
    ...
    async def _process_candidates(self, candidates: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for candidate in candidates:
            ...
            if direction not in {"long", "short"}:
                ...
                continue
            session = contract_session_status(
                contract,
                str(candidate.get("contract_type", "")),
                self.settings,
            )
            if session["is_stock"] and not session["is_open"]:
                actions.append(
                    {
                        "contract": contract,
                        "status": "skipped_stock_market_closed",
                        "direction": direction,
                        "reason": "underlying stock exchange is outside its regular session",
            ...
            if contract in positions_by_contract:
                ...
                # 30m candles and the held position must independently lose
                # its closed-15m thesis before a full flip. Scanner direction
                # alone is never allowed to close an existing position.
                    ...
                    actions.append(
                        {
                            ...
                            "reason": "opposite 30m ranking and held-position 15m thesis loss must both confirm before position flip",
                            "confirmations": reversal["confirmations"],
                            "required_confirmations": reversal["required"],
                            "observation": reversal["observation"],
                            "position_thesis_ready": reversal.get(
                                "position_thesis_ready", False
                            ),
                        }
                    )
                    continue
    ...
    async def _confirm_reversal_signal(
        ...
        """Require both persistent 30m opposition and the held coin's 15m loss."""
        required = max(
            2,
        ...
        await self.repository.save_managed_position(record)
        thesis_ready = bool(
            plan.get("thesis_failure_confirmed")
            or str(plan.get("position_thesis_state", "")).upper()
            == "EXIT_ARMED"
        )
        return {
            "confirmed": confirmations >= required and thesis_ready,
            "confirmations": confirmations,
            "required": required,
            "observation": observation,
            "position_thesis_ready": thesis_ready,
        }
    ...
    def _five_minute_entry_price(
        ...
        *,
        ...
        """Anchor every passive entry to closed 5m structure.

        The 5m frame chooses price, while 30m/4h still own the trade thesis,
        stop and target horizon. This prevents a fixed offset from chasing a
        fast altcoin candle without turning entry timing into a new strategy.
        """
        frame5 = metrics.get("5m", {}) if isinstance(metrics, dict) else {}
        atr5 = _number(frame5.get("atr"))
        if atr5 <= 0:
            raise TradingRiskError(
                "FIVE_MINUTE_ENTRY_DATA_UNAVAILABLE",
                "closed 5m ATR is required for limit-entry placement",
            )
        references = {
            name: _number(frame5.get(name))
            for name in ("ema20", "ema50", "vwap", "recent_low", "recent_high")
        }
        support_names = (
            ("ema20", "ema50", "vwap", "recent_low")
            if side == "long"
            else ("ema20", "ema50", "vwap", "recent_high")
        )
        usable = {
        ...
        if not usable:
            raise TradingRiskError(
                "FIVE_MINUTE_ENTRY_DATA_UNAVAILABLE",
                "closed 5m EMA/VWAP/structure references are required",
            )
        passive = cls._limit_entry_price(
            ticker,
            side,
            mark_price,
            info,
            float(settings.limit_entry_offset_pct),
        )
        maximum_distance = min(
            mark_price * float(settings.five_minute_entry_max_distance_pct),
            atr5 * float(settings.five_minute_entry_max_atr),
        )
        if maximum_distance <= 0:
            raise TradingRiskError(
                "INVALID_FIVE_MINUTE_ENTRY_RANGE",
                "5m entry distance is not positive",
            )
        if side == "long":
            in_range = {
                name: value
                for name, value in usable.items()
                if mark_price - maximum_distance <= value <= mark_price
            }
            if in_range:
                source, anchor = max(in_range.items(), key=lambda item: item[1])
                raw_price = min(passive, anchor)
            else:
                source = "atr_fallback"
                raw_price = min(
                    passive,
                    mark_price
                    - atr5 * float(settings.five_minute_entry_fallback_atr),
                )
            raw_price = max(mark_price - maximum_distance, raw_price)
        else:
            in_range = {
                name: value
                for name, value in usable.items()
                if mark_price <= value <= mark_price + maximum_distance
            }
            if in_range:
                source, anchor = min(in_range.items(), key=lambda item: item[1])
                raw_price = max(passive, anchor)
            else:
                source = "atr_fallback"
                raw_price = max(
                    passive,
            ...
            raw_price = min(mark_price + maximum_distance, raw_price)
        tick = _number(getattr(info, "order_price_round", None))
        if tick > 0:
            units = Decimal(str(raw_price)) / Decimal(str(tick))
            rounding = ROUND_FLOOR if side == "long" else ROUND_CEILING
            raw_price = float(
                units.to_integral_value(rounding=rounding) * Decimal(str(tick))
            )
        if raw_price <= 0:
            raise TradingRiskError(
                "INVALID_LIMIT_PRICE", "5m structure produced an invalid price"
            )
        return raw_price, {
            "model": "closed_5m_structure",
            "source": source,
            "atr5": atr5,
    ...
    async def _open_candidate(self, candidate: dict[str, Any], info: Any) -> dict[str, Any]:
        ...
        if session["is_stock"] and not session["is_open"]:
            raise TradingRiskError(
                "STOCK_MARKET_CLOSED",
                f"{info.name} underlying market is closed ({session['calendar']})",
            )
        candidate_metrics = candidate.get("metrics", {})
        limit_price, entry_model = self._five_minute_entry_price(
            ticker=ticker,
        ...
        ticker_metrics = {
            **candidate_metrics,
            "ticker": {**candidate_metrics.get("ticker", {}), **ticker},
            "signal_class": candidate.get("signal_class", "trend"),
        }
        ...
        if actual_size <= 0:
            ...
            action = {
                ...
                "signal_class": plan.get("signal_class"),
                "entry_model": entry_model,
            }
            return action
        self._pending_entry_metadata.pop(entry_order_id, None)
    ...
    async def _monitor_pending_entries(self) -> list[dict[str, Any]]:
        ...
        for order in pending:
            ...
            session = contract_session_status(
                contract,
                str(metadata.get("contract_type", "")),
                self.settings,
            )
            if session["is_stock"] and not session["is_open"]:
                reason = "STOCK_MARKET_CLOSED"
            elif age >= int(self.settings.limit_entry_timeout_seconds):
    ...
    async def manage_once(self) -> dict[str, Any]:
        """Run one complete position cycle without racing scanner order work."""
        async with self._order_lock:
            return await self._manage_once_unlocked()

    async def _manage_once_unlocked(self) -> dict[str, Any]:
        positions = await self.gate.rest.get_positions()
        active = {
            str(item.get("contract", "")).upper(): item
            for item in positions
    ...
    async def _manage_position(
        ...
        prior_thesis_state = str(
            plan.get("position_thesis_state", "HEALTHY")
        ).upper()
        ...
        trend_confirmation = self._update_thesis_failure_confirmation(
            ...
            int(
                getattr(
                    ...
                    getattr(
                        self.settings,
                        "thesis_soft_failure_confirmations",
                        2,
                    ),
                )
            ),
        ...
        plan["trend_observation_15m"] = trend_assessment.get(
            "observation_key"
        )
        now = _now()
        opened_at = self._as_utc_datetime(plan.get("opened_at")) or now
        ...
        def arm_five_minute_exit(reason: str) -> bool:
            if str(plan.get("position_thesis_state", "")).upper() == "EXIT_ARMED":
                return False
            plan["position_thesis_state"] = "EXIT_ARMED"
            plan["exit_armed_at"] = now.isoformat()
            plan["exit_armed_observation_15m"] = trend_assessment.get(
                "observation_key"
            )
            plan["exit_reason"] = reason
            plan["exit_recovery_target"] = self._exit_recovery_target(
                plan=plan,
                entry=entry,
                price=price,
                current_r=current_r,
            )
            plan["exit_5m_confirmations"] = 0
            plan.pop("exit_5m_observation", None)
            plan["phase"] = "EXIT_ARMED_5M"
            return True

        if (
            prior_thesis_state == "EXIT_ARMED"
            and bool(trend_assessment.get("healthy"))
        ):
            plan["position_thesis_state"] = "HEALTHY"
            plan["phase"] = "INITIAL_RISK"
            for key in (
                "exit_armed_at",
                "exit_armed_observation_15m",
                "exit_reason",
                "exit_recovery_target",
                "exit_5m_confirmations",
                "exit_5m_observation",
            ):
                plan.pop(key, None)
            state_events.append(
                {
                    "contract": plan["contract"],
                    "status": "position_exit_disarmed",
                    "reason": "closed 15m trend evidence recovered before a 5m exit trigger",
                    "side": plan["side"],
                    "current_r": current_r,
                    "thesis_evidence": trend_assessment["evidence"],
                }
            )
        elif bool(trend_confirmation["confirmed"]):
            if arm_five_minute_exit("confirmed_15m_thesis_loss"):
                state_events.append(
                    {
                        "contract": plan["contract"],
                        "status": "position_exit_armed",
                        "reason": "multi-factor closed 15m evidence confirms the held trend has disappeared",
                        "side": plan["side"],
                        "current_r": current_r,
                        "recovery_target": plan["exit_recovery_target"],
                        "thesis_score": trend_break_score,
                        "thesis_evidence": trend_assessment["evidence"],
                        "thesis_confirmations": trend_confirmation[
                            "confirmations"
                        ],
                    }
                )
            ...
            prior_thesis_state != "EXIT_ARMED"
            and bool(trend_assessment.get("candidate"))
        ):
            plan["position_thesis_state"] = "WATCH"
            if prior_thesis_state != "WATCH":
                state_events.append(
                    {
                        "contract": plan["contract"],
                        "status": "position_thesis_watch",
                        "reason": "one closed 15m observation is weak; no exit until independent confirmation",
                        "side": plan["side"],
                        "current_r": current_r,
                        "thesis_score": trend_break_score,
                        "thesis_evidence": trend_assessment["evidence"],
        ...
        elif prior_thesis_state != "EXIT_ARMED":
            plan["position_thesis_state"] = (
                "HEALTHY"
                if trend_assessment.get("healthy")
                else "NEUTRAL"
            )
            if (
                prior_thesis_state == "WATCH"
                and plan["position_thesis_state"] == "HEALTHY"
            ):
                state_events.append(
                    {
                        "contract": plan["contract"],
                        "status": "position_thesis_recovered",
                        "reason": "15m weakness did not persist and was treated as noise",
                        "side": plan["side"],
                        "current_r": current_r,
                    }
                )

        # A regime clock is an opportunity-cost input, never a clock-only
        # market exit. Healthy 15m evidence extends the thesis. Stale/neutral
        # evidence may arm the same 5m execution state used by a reversal.
        if time_remaining_hours <= 0:
            observation15 = str(trend_assessment.get("observation_key") or "")
            if (
                bool(trend_assessment.get("healthy"))
                or current_r >= float(self.settings.time_stop_min_progress_r)
            ):
                deadline = now + timedelta(
                    hours=float(
                        self.settings.time_stop_confirmation_extension_hours
                    )
                )
                plan["time_stop_deadline"] = deadline.isoformat()
                ...
                if (
                    plan.get("last_time_extension_observation")
                    != observation15
                ):
                    plan["last_time_extension_observation"] = observation15
                    state_events.append(
                        {
                            "contract": plan["contract"],
                            "status": "time_decay_grace",
                            "reason": "closed 15m trend remains healthy or has made sufficient progress",
                            "side": plan["side"],
                            "current_r": current_r,
            ...
            elif arm_five_minute_exit("time_decay_with_weak_15m_thesis"):
                state_events.append(
                    {
                        "contract": plan["contract"],
                        "status": "position_exit_armed",
                        "reason": "holding window expired without progress and closed 15m trend is no longer healthy",
                        "side": plan["side"],
                        "current_r": current_r,
                        "recovery_target": plan["exit_recovery_target"],
                        "thesis_score": trend_break_score,
        ...
        if state_events:
            # Persist the transition before announcing it. This makes the
            # stored position state the source of truth if notification
            # delivery fails or the process restarts immediately afterward.
            record["plan"] = plan
        ...
        for state_event in state_events:
            try:
                await self._record_position_event(state_event)
            except Exception as exc:
                logger.warning(
                    "position state notification failed for %s: %s",
                    plan["contract"],
                    type(exc).__name__,
                )

        exit5_assessment: dict[str, Any] = {}
        exit5_confirmation: dict[str, int | bool] = {
            "confirmations": 0,
            "required": int(
                getattr(self.settings, "position_exit_5m_confirmations", 2)
            ),
            "confirmed": False,
        }
        if str(plan.get("position_thesis_state", "")).upper() == "EXIT_ARMED":
            exit5_assessment = assess_closed_5m_exit(
                side=plan["side"],
                context=context,
                live_price=price,
                current_r=current_r,
                recovery_target=_number(plan.get("exit_recovery_target")),
                settings=self.settings,
            )
            exit5_confirmation = self._update_5m_exit_confirmation(
                plan,
                exit5_assessment,
                int(
                    getattr(
                        self.settings, "position_exit_5m_confirmations", 2
                    )
                ),
            )
            close_reason = (
                "recovered_to_planned_exit"
                if exit5_assessment.get("target_reached")
                else "risk_accelerated_after_confirmed_15m_loss"
                if exit5_assessment.get("emergency")
                else "decisive_5m_adverse_continuation"
                if exit5_assessment.get("decisive")
                else "confirmed_5m_adverse_continuation"
                if exit5_confirmation.get("confirmed")
                else ""
            )
            if close_reason:
                try:
                    await self._close_for_trend_break(
                        plan["contract"], position
                    )
                    record["status"] = "closed"
                    record["closed_at"] = now.isoformat()
                    plan["phase"] = "FIVE_MINUTE_EXIT"
                    plan["position_thesis_state"] = "CLOSED"
                    record["plan"] = plan
                    record["updated_at"] = now.isoformat()
                    await self.repository.save_managed_position(record)
                    action = {
                        "contract": plan["contract"],
                        "status": "position_exit_executed",
                        "reason": close_reason,
                        "side": plan["side"],
                        "current_r": current_r,
                        "recovery_target": plan.get(
                            "exit_recovery_target"
                        ),
                        "thesis_score": trend_break_score,
                        "thesis_evidence": trend_assessment["evidence"],
                        ...
                        "exit_5m_score": exit5_assessment.get("score"),
                        "exit_5m_evidence": exit5_assessment.get(
                            "evidence", []
                        ),
                        "exit_5m_confirmations": exit5_confirmation[
                            "confirmations"
                        ],
                        "protection_status": "exchange_cleanup_before_close",
                    }
                    try:
                        await self._record_position_event(action)
                    except Exception as exc:
                        logger.warning(
                            "position exit notification failed for %s: %s",
                            plan["contract"],
                            type(exc).__name__,
                        )
                    return action
                    ...
                    protection_error = (
                        protection_error
                        or f"position_exit:{type(exc).__name__}: {exc}"
                    )
                    logger.exception(
                        "confirmed 15m/5m position exit failed for %s",
                        plan["contract"],
                    )
        reached_stages = [
            target["stage"]
        ...
        plan["closed_favorable_extreme"] = _number(
            trail.get("closed_favorable_extreme"),
            _number(plan.get("closed_favorable_extreme"), entry),
        )
        plan["favorable_close_extreme"] = _number(
            trail.get("favorable_close_extreme"),
            _number(plan.get("favorable_close_extreme"), entry),
        )
        ...
        result = {
            ...
            "position_thesis_state": plan.get("position_thesis_state"),
            "trend_observation_15m": plan.get("trend_observation_15m"),
            "thesis_evidence": trend_assessment.get("evidence", []),
            "exit_recovery_target": plan.get("exit_recovery_target"),
            "exit_5m_score": exit5_assessment.get("score"),
            "exit_5m_confirmations": exit5_confirmation.get(
                "confirmations", 0
            ),
            "holding_hours": round(holding_hours, 2),
    ...
    async def _management_loop(self) -> None:
        while self._running:
            ...
            try:
                ...
                # Pending-entry cancellation, position exits, protection
                # replacement and scanner submissions all mutate the same
                # exchange order state.  Serialize the whole maintenance
                # decision so no task can act on a half-updated position.
                async with self._order_lock:
                    try:
                        pending_actions = await self._monitor_pending_entries()
                    except Exception:
                        logger.exception("pending entry monitoring cycle failed")
                    await self._manage_once_unlocked()
    ...
    async def _market_context(self, contract: str, info: Any) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        required15 = max(
            288,
            int(getattr(self.settings, "position_trend_15m_candles", 288)),
        )
        required5 = max(
            96,
            int(getattr(self.settings, "position_exit_5m_candles", 96)),
        )
        raw15, raw5 = await asyncio.gather(
            self.gate.rest.get_candlesticks(
                contract, "15m", limit=required15 + 40
            ),
            self.gate.rest.get_candlesticks(
                contract, "5m", limit=required5 + 40
            ),
        )
        ...
        integrity15 = candle_integrity(candles15, "15m", required15)
        integrity5 = candle_integrity(candles5, "5m", required5)
        if not integrity15["complete"] or not integrity5["complete"]:
            problems = sorted(
                set(integrity15["problems"]) | set(integrity5["problems"])
            )
            raise TradingRiskError(
                "POSITION_CANDLE_DATA_INVALID",
                f"{contract} closed 15m/5m data invalid: {','.join(problems)}",
            )
        context: dict[str, Any] = {
            "integrity15": integrity15,
            "integrity5": integrity5,
            "source": "closed_15m_position_monitor",
        }

        def trailing_count(values: pd.Series) -> int:
            count = 0
        ...
        for label, candles in (("15", candles15), ("5", candles5)):
            ...
            required = required15 if label == "15" else required5
            frame = frame.tail(required).reset_index(drop=True)
            ...
            dmi = dmi_adx(high, low, close)
            volume = pd.to_numeric(
                frame.get("turnover_usdt"), errors="coerce"
            )
            fallback_volume = pd.to_numeric(
                frame.get("volume_contracts"), errors="coerce"
            )
            volume = volume.fillna(fallback_volume).fillna(0.0)
            vwap_series = vwap(high, low, close, volume)
            mfi_series = mfi(high, low, close, volume)
            turnover = turnover_features(volume)
            context[f"atr{label}"] = atr_value
            ...
            context[f"last_high{label}"] = float(high.iloc[-1])
            context[f"last_low{label}"] = float(low.iloc[-1])
            context[f"ema20{label}"] = float(ema20_series.iloc[-1])
            context[f"ema50{label}"] = float(ema50_series.iloc[-1])
            context[f"vwap{label}"] = (
                float(vwap_series.iloc[-1])
                if pd.notna(vwap_series.iloc[-1])
                else None
            )
            context[f"plus_di{label}"] = (
                float(dmi["plus_di"].iloc[-1])
                if pd.notna(dmi["plus_di"].iloc[-1])
                else None
            )
            context[f"minus_di{label}"] = (
                float(dmi["minus_di"].iloc[-1])
                if pd.notna(dmi["minus_di"].iloc[-1])
                else None
            )
            context[f"adx{label}"] = (
                float(dmi["adx"].iloc[-1])
                if pd.notna(dmi["adx"].iloc[-1])
                else None
            )
            context[f"mfi{label}"] = (
                float(mfi_series.iloc[-1])
                if pd.notna(mfi_series.iloc[-1])
                else None
            )
            context[f"turnover_ratio{label}"] = (
                float(turnover["turnover_ratio"].iloc[-1])
                if pd.notna(turnover["turnover_ratio"].iloc[-1])
                else None
            )
            context[f"ema20_slope_atr{label}"] = (
                float(
                    (ema20_series.iloc[-1] - ema20_series.iloc[-4])
                    / atr_value
                )
                if atr_value and len(ema20_series) >= 4
                else None
            )
            below_trend = (close < ema20_series) & (
                vwap_series.isna() | (close < vwap_series)
            )
            above_trend = (close > ema20_series) & (
                vwap_series.isna() | (close > vwap_series)
            )
            context[f"below_trend_count{label}"] = trailing_count(
                below_trend
            )
            context[f"above_trend_count{label}"] = trailing_count(
                above_trend
            )
    ...
    def _trend_break_assessment(
        ...
        """Compatibility façade for the closed-15m position thesis model."""
        assessment = assess_closed_15m_thesis(
            str(plan.get("side", "")).lower(),
            context,
            self.settings,
        )
        assessment["hard_failure"] = bool(assessment.get("decisive"))
        assessment["soft_failure"] = bool(
            assessment.get("candidate") and not assessment.get("decisive")
        )
        assessment["failure_kind"] = (
            "decisive_15m_reversal"
            if assessment.get("decisive")
            else "confirmed_15m_deterioration"
            if assessment.get("candidate")
            else "none"
        )
        assessment["profitable_weakness"] = bool(
            int(assessment.get("score", 0)) >= 4
        )
        return assessment
    ...
    def _update_thesis_failure_confirmation(
        ...
        """Count distinct completed 15m observations, never manager ticks."""
        ...
        decisive = bool(
            assessment.get("decisive") or assessment.get("hard_failure")
        )
        required = 1 if decisive else max(2, int(soft_required))
        if decisive:
    ...
    @staticmethod
    def _update_5m_exit_confirmation(
        plan: dict[str, Any],
        assessment: dict[str, Any],
        required: int,
    ) -> dict[str, int | bool]:
        observation = str(assessment.get("observation_key") or "")
        previous = str(plan.get("exit_5m_observation") or "")
        confirmations = int(_number(plan.get("exit_5m_confirmations"), 0))
        if observation and observation != previous:
            if assessment.get("candidate"):
                confirmations += 1
            else:
                confirmations = 0
            plan["exit_5m_observation"] = observation
        decisive = bool(assessment.get("decisive"))
        if decisive:
            confirmations = max(confirmations, 1)
        minimum = 1 if decisive else max(2, int(required))
        confirmed = bool(assessment.get("candidate")) and (
            confirmations >= minimum
        )
        plan["exit_5m_confirmations"] = confirmations
        return {
            "confirmations": confirmations,
            "required": minimum,
            "confirmed": confirmed,
    ...
    def _exit_recovery_target(
        ...
        *,
        ...
        risk_distance = _number(plan.get("initial_risk_distance"))
        if current_r >= 0 or risk_distance <= 0:
            return price
        maximum_loss_r = min(
            0.0,
            float(
        ...
        target_r = 0.0 if current_r >= maximum_loss_r else maximum_loss_r
        fee_distance = entry * float(
            getattr(self.settings, "estimated_round_trip_fee_pct", 0.0012)
        )
        if plan["side"] == "long":
            return entry + target_r * risk_distance + fee_distance
        return entry - target_r * risk_distance - fee_distance

    async def _record_position_event(self, action: dict[str, Any]) -> None:
        await self.repository.save_order_event(
            {
                "event_id": uuid.uuid4().hex,
                "client_order_id": None,
                "contract": action.get("contract"),
                "event_type": action.get("status", "position_event"),
                "created_at": _now(),
                "payload": action,
            }
        )
    ...
    def _trend_break_score(
        ...
        return int(self._trend_break_assessment(plan, context)["score"])
    ...
    def _managed_payload(
        ...
        plan.setdefault("position_thesis_state", "HEALTHY")
        plan.setdefault("thesis_failure_confirmations", 0)
        plan.setdefault("exit_5m_confirmations", 0)
        plan.setdefault(
            "time_stop_deadline",
    ...
    def _time_stop_horizon_hours(plan: dict[str, Any], settings: Any | None) -> float:
        if (
            ...
            and str(plan.get("signal_class", "")).lower() == "tactical"
        ):
            return max(3.0, float(settings.tactical_time_stop_hours))
