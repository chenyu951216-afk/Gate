...
function renderPositions(overview) {
  ...
  const rows = overview.positions.map((position) => {
    ...
    return `<tr><td>${escapeHtml(position.contract)}</td><td class="${position.side === 'LONG' ? 'profit' : 'loss'}">${position.side}</td><td>${numberText(position.size, 4)}</td><td>${numberText(position.entry_price, 6)}</td><td>${numberText(position.mark_price, 6)}</td><td>${numberText(position.leverage, 1)}x</td><td class="${pnlClass}">${signedNumberText(position.unrealised_pnl)} USDT</td><td>${signedNumberText(position.pnl_percent)}%</td><td>${numberText(position.liquidation_price, 6)}</td><td>${numberText(position.margin)} USDT</td><td>${protectionByContract[position.contract] || 0}</td></tr>`;
  }).join('');
  holder.innerHTML = `<table class="data-table"><thead><tr><th>合約</th><th>方向</th><th>數量</th><th>進場價</th><th>標記價</th><th>槓桿</th><th>未實現損益</th><th>損益%</th><th>清算價</th><th>保證金</th><th>保護單</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
async function loadOverview() {
  ...
  try {
    ...
    if (positionsSummary) positionsSummary.textContent = `Gate 持倉 ${data.summary?.position_count || 0} 個；未實現損益 ${signedNumberText(data.summary?.unrealised_pnl)} USDT；保護單 ${data.summary?.protection_order_count || 0} 張`;
    if (message) {
