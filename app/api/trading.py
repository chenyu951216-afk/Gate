...
async def trading_overview(request: Request) -> dict[str, Any]:
    ...
    results_raw: tuple[Any, Any, Any, Any, Any] = await asyncio.gather(
        ...
        state.bitget.rest.get_order_fills(limit=100),
        return_exceptions=True,
    )
    account_result, positions_result, open_orders_result, protection_orders_result, fills_result = results_raw
    results = {
        "account": account_result,
        "positions": positions_result,
        "open_orders": open_orders_result,
        "protection_orders": protection_orders_result,
        "recent_fills": fills_result,
    }
    errors = {
    ...
    recent_fills = results["recent_fills"] if isinstance(results["recent_fills"], list) else []
    pnl_by_contract: dict[str, dict[str, float]] = {}
    for fill in recent_fills:
        contract = str(fill.get("contract") or "UNKNOWN")
        item = pnl_by_contract.setdefault(
            contract, {"realized_pnl": 0.0, "fees": 0.0, "net_pnl": 0.0, "fills": 0.0}
        )
        item["realized_pnl"] += float(fill.get("realized_pnl") or 0)
        item["fees"] += float(fill.get("fee") or 0)
        item["net_pnl"] += float(fill.get("net_pnl") or 0)
        item["fills"] += 1
    ...
    return {
        ...
        "recent_pnl_by_contract": pnl_by_contract,
        ...
        "summary": {
            ...
            "recent_realized_pnl": sum(float(item.get("realized_pnl") or 0) for item in recent_fills),
            "recent_fees": sum(float(item.get("fee") or 0) for item in recent_fills),
            "recent_net_pnl": sum(float(item.get("net_pnl") or 0) for item in recent_fills),
        },
    }
