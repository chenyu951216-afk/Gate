...
async def trading_overview(request: Request) -> dict[str, Any]:
    await require_bearer(request)
    state = state_from_request(request)
    if not (state.settings.gate_api_key and state.settings.gate_api_secret):
        return {"gate_status": "not_configured", "account": {}, "positions": [], "open_orders": [], "protection_orders": [], "managed": []}
    account_result, positions_result, open_orders_result, protection_orders_result = await asyncio.gather(
        state.gate.rest.get_account(),
        state.gate.rest.get_positions(),
        state.gate.rest.get_open_orders(limit=100),
        state.gate.rest.get_price_orders(status="open", limit=100),
    ...
    return {
        "gate_status": "partial_error" if errors else "connected",
        "gate_errors": errors,
