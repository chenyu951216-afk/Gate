...
async def initialize_database(engine: AsyncEngine | None) -> None:
    ...
    async with engine.begin() as connection:
        ...
        # Existing Zeabur databases were created before the live/test control
        # existed.  Keep startup backwards-compatible without requiring a
        # separate migration deployment.
        await connection.execute(text("ALTER TABLE trading_control ADD COLUMN IF NOT EXISTS mode VARCHAR(16) NOT NULL DEFAULT 'live'"))
