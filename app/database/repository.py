...
class MemoryRepository:
    ...
    async def get_trading_control(self) -> dict[str, Any]:
        return {"paused": self.trading_paused, "reason": self.trading_pause_reason}
...
class PostgresRepository(MemoryRepository):
    ...
    async def get_trading_control(self) -> dict[str, Any]:
        async with self.session_factory() as session:
            ...
            if row is None:
                row = TradingControl(id=1, paused=False, updated_at=datetime.now(timezone.utc))
                session.add(row)
                await session.commit()
    ...
    async def set_trading_paused(self, paused: bool, reason: str | None = None) -> dict[str, Any]:
        async with self.session_factory() as session:
            row = await session.get(TradingControl, 1)
            if row is None:
                row = TradingControl(id=1, paused=paused, reason=reason, updated_at=datetime.now(timezone.utc))
                session.add(row)
            else:
                row.paused = paused
                row.reason = reason
