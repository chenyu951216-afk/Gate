...
def _history_features(payload: Any) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        normalized.append(
            {
                # CoinGlass documents milliseconds but some responses have
                # historically returned Unix seconds.  Normalize both so the
                # scan snapshot and cached context have one time unit.
                "time": (timestamp * 1000 if timestamp and timestamp < 10_000_000_000 else timestamp) or 0.0,
...
def _heatmap_features(payload: Any, current_price: float | None) -> dict[str, Any]:
    ...
    if isinstance(root, Mapping) and isinstance(root.get("data"), Mapping):
        root = root["data"]
    if isinstance(root, Mapping) and isinstance(root.get("liqMapV2"), Mapping):
        root = root["liqMapV2"]
    if not isinstance(root, (Mapping, list)):
        raise CoinGlassAPIError("CoinGlass liquidation heatmap has invalid data")
    levels: list[dict[str, float]] = []

    def visit(node: Any, inherited_price: float | None = None) -> None:
        if isinstance(node, Mapping):
            for key, value in node.items():
                price = _number(key) or inherited_price
                if price is not None:
                    ...
                    for entry in entries:
                        if isinstance(entry, list) and len(entry) >= 2:
                            ...
                            level_price = price
                            usd = _number(entry)
                        if level_price is not None and usd is not None and usd > 0:
                            levels.append({"price": level_price, "usd": usd})
                # Official aggregated-map responses can wrap the numeric
                # price map in `data`, `liqMapV2`, or another model key.
                if not _number(key):
                    visit(value, inherited_price)
        elif isinstance(node, list):
            for entry in node:
                visit(entry, inherited_price)

    visit(root)
    deduped: dict[tuple[float, float], dict[str, float]] = {
        (item["price"], item["usd"]): item for item in levels
    }
    levels = list(deduped.values())
