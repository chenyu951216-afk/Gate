...
def _positive(value: Any, default: float = 0.0) -> float:
    try:
        ...
        return default
    return result if result > 0 and isfinite(result) else default
...
class BitgetRestClient:
    ...
    async def request(
        ...
        async with self.semaphore:
            for attempt in range(attempts):
                ...
                try:
                    # A retry must receive a fresh timestamp and signature.
                    # Reusing the first signature after rate-limit/network
                    # backoff can turn a recoverable request into an auth
                    # timestamp rejection.
                    headers = (
                        self._auth_headers(method, path, query, body)
                        if authenticated
                        else {}
                    )
                    ...
                    # Business/parameter/auth errors are deterministic client
                    # responses, not an unstable Bitget service. Counting
                    # them would let one unsupported symbol open the global
                    # circuit breaker and block unrelated valid contracts.
                    raise
    ...
    def _normalize_contract(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        symbol_type = str(raw.get("symbolType", "perpetual")).lower()
        supported_margin = raw.get("supportMarginCoins")
        margin_supported = (
            not isinstance(supported_margin, list)
            or not supported_margin
            or self.margin_coin in {str(item).upper() for item in supported_margin}
        )
        api_tradable = (
            status == "normal"
            and symbol_type == "perpetual"
            and margin_supported
        )
        ...
        result = {
            ...
            "status": "trading" if api_tradable else "delisted",
            "type": "direct",
            "quanto_multiplier": 1,
            "contract_size": 1,
            "leverage_min": _positive(raw.get("minLever"), 1),
            "leverage_max": _positive(raw.get("maxLever"), 0),
            ...
            "order_notional_min": _positive(raw.get("minTradeUSDT"), 0),
            "enable_decimal": True,
            ...
            "symbolType": raw.get("symbolType"),
            "supportMarginCoins": supported_margin,
            "sizeMultiplier": raw.get("sizeMultiplier"),
            "minTradeNum": raw.get("minTradeNum"),
            "minTradeUSDT": raw.get("minTradeUSDT"),
    ...
    async def get_max_openable_quantity(
        self,
        contract: str,
        side: str,
        price: float,
    ) -> float:
        """Return Bitget's account-aware maximum opening base quantity."""
        payload = await self.request(
            "GET",
            "/mix/account/max-open",
            params={
                "symbol": self._symbol(contract),
                "productType": self.product_type,
                ...
                "orderType": "limit",
                "openPrice": _text(price),
            },
            authenticated=True,
        )
        if not isinstance(payload, Mapping):
            return 0.0
        return _positive(payload.get("maxOpen"))

    async def get_order_book(self, contract: str, limit: int = 50) -> dict[str, Any]:
        """Return normalized Bitget futures depth from the official v2 API."""
        payload = await self.request(
            "GET",
        ...
        def levels(value: Any) -> list[dict[str, float]]:
            result: list[dict[str, float]] = []
            for row in value if isinstance(value, list) else []:
                if not isinstance(row, (list, tuple)) or len(row) < 2:
                    continue
                price, size = _positive(row[0]), _positive(row[1])
                if price > 0 and size > 0:
                    result.append({"price": price, "size": size, "notional": price * size})
            return result

        return {
            "contract": contract,
            "bids": levels(root.get("bids")),
            "asks": levels(root.get("asks")),
    ...
    async def get_order_fills(
        ...
        rows = payload.get("fillList", []) if isinstance(payload, Mapping) else []
        result: list[dict[str, Any]] = []
        for row in rows:
            if not isinstance(row, Mapping):
                continue
            ...
            if isinstance(fee_detail, list):
                for item in fee_detail:
                    if isinstance(item, Mapping):
                        try:
                            fee += float(item.get("totalFee") or 0)
                        except (TypeError, ValueError):
                            continue
            ...
            result.append(
                {
                    ...
                    "contract": self._contract(str(row.get("symbol", ""))),
                    "side": row.get("side"),
                    "trade_side": row.get("tradeSide"),
                    "price": _positive(row.get("price")),
                    "size": _positive(row.get("baseVolume")),
    ...
    async def set_leverage(self, contract: str, leverage: float, margin_mode: str = "cross", dual_side: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise GateAPIError(
                f"invalid leverage {leverage!r}",
                endpoint="/mix/account/set-leverage",
            ) from exc
        if not leverage_value.is_finite():
            raise GateAPIError(
                f"invalid leverage {leverage!r}",
                endpoint="/mix/account/set-leverage",
            )
        leverage_integer = int(
            leverage_value.to_integral_value(rounding=ROUND_DOWN)
        )
        if leverage_integer < 1:
            raise GateAPIError(
        ...
        return {
            ...
            "leverage": result.get("crossMarginLeverage") or result.get("leverage") or str(leverage_integer),
            "cross_leverage_limit": result.get("crossMarginLeverage") or str(leverage_integer),
        }
    ...
    async def place_futures_order(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            signed_decimal = Decimal(str(body.get("size") or 0))
        except Exception as exc:
            raise GateAPIError(
                f"invalid order size for {contract}",
                endpoint="/mix/order/place-order",
            ) from exc
        ...
        if signed == 0:
            raise GateAPIError(
                f"order size is zero for {contract}",
                endpoint="/mix/order/place-order",
            )
        ...
        if order_type == "limit":
            try:
                price_decimal = Decimal(str(body.get("price")))
            except Exception as exc:
                raise GateAPIError(
                    f"invalid limit price for {contract}",
                    endpoint="/mix/order/place-order",
                ) from exc
        ...
        if contract_info:
            if str(contract_info.get("status", "")).lower() != "trading":
                raise GateAPIError(
                    f"contract is not open for API trading: {contract}",
                    endpoint="/mix/order/place-order",
                )
            raw_contract = contract_info.get("raw", {}) or {}
            quantity = abs(signed_decimal)
            step = Decimal(
                str(
                    contract_info.get("sizeMultiplier")
                    or raw_contract.get("sizeMultiplier")
                    or "0"
                )
            )
            minimum = Decimal(
                str(
                    contract_info.get("order_size_min")
                    or raw_contract.get("minTradeNum")
                    or "0"
                )
            )
            maximum_key = "maxOrderQty" if order_type == "limit" else "maxMarketOrderQty"
            maximum = Decimal(str(raw_contract.get(maximum_key) or "0"))
            if step > 0 and quantity % step != 0:
                raise GateAPIError(
                    f"order size {quantity} is not a multiple of {step} for {contract}",
                    endpoint="/mix/order/place-order",
                )
            if minimum > 0 and quantity < minimum and not reduce_only:
                raise GateAPIError(
                    f"order size {quantity} is below minimum {minimum} for {contract}",
                    endpoint="/mix/order/place-order",
                )
            if maximum > 0 and quantity > maximum:
                raise GateAPIError(
                    f"order size {quantity} exceeds maximum {maximum} for {contract}",
                    endpoint="/mix/order/place-order",
                )
            minimum_notional = Decimal(
                str(
                    contract_info.get("order_notional_min")
                    or raw_contract.get("minTradeUSDT")
                    or "0"
                )
            )
            if (
                ...
                and quantity * price_decimal < minimum_notional
            ):
                raise GateAPIError(
                    (
                        f"order notional {quantity * price_decimal} is below "
                        f"minimum {minimum_notional} for {contract}"
                    ),
                    endpoint="/mix/order/place-order",
                )
            tick = Decimal(
                str(contract_info.get("order_price_round") or "0")
            )
            if order_type == "limit" and tick > 0 and price_decimal % tick != 0:
                raise GateAPIError(
                    f"limit price {price_decimal} is not a multiple of {tick} for {contract}",
                    endpoint="/mix/order/place-order",
                )
    ...
    async def create_price_order(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        requested_size = abs(float(initial.get("size") or initial.get("amount") or 0))
        position = None
        try:
            position = await self.get_position(contract)
        except Exception as exc:
            # A just-filled order can briefly exist before the position
            # endpoint catches up. Use the confirmed fill size supplied by
            # the caller instead of turning the protection size into zero.
            logger.warning("Bitget position read delayed before protection %s: %s", contract, type(exc).__name__)
        live_size = abs(float(position.get("size", 0))) if is_stop and position else 0.0
        size = live_size if live_size > 0 else requested_size
        contract_info = self._contracts.get(contract.upper())
        ...
        client_oid = result.get("clientOid") or initial.get("text")
        return {"id": order_id, "id_string": order_id, "clientOid": client_oid, "text": client_oid, "raw": result}
    ...
    async def get_price_orders(
        ...
        for row in rows:
            ...
            # Bitget's pending response uses triggerPrice/triggerType for the
            # plan itself.  Keep the explicit TP/SL fields as a fallback for
            # account variants that return those fields instead.
            if trigger_price in (None, ""):
                trigger_price = (
                    row.get("stopLossTriggerPrice")
                    if plan_type == "loss_plan"
                    else row.get("stopSurplusTriggerPrice")
                )
            if trigger_type in (None, ""):
                trigger_type = (
                    row.get("stopLossTriggerType")
                    if plan_type == "loss_plan"
                    else row.get("stopSurplusTriggerType")
                )
            result.append({
                "id": row.get("orderId"),
                "id_string": row.get("orderId"),
                ...
                "trigger": {"price": trigger_price, "price_type": 1 if trigger_type == "mark_price" else 0},
                "text": row.get("clientOid"),
                "clientOid": row.get("clientOid"),
                "margin_mode": "cross" if str(row.get("marginMode", "crossed")).lower() == "crossed" else "isolated",
