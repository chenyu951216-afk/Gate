...
def analyze_market(
    ...
    integrity = collected.get("data_integrity", {})
    available = {
    ...
    def build(frame: pd.DataFrame, include_mfi: bool) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            "closed_timestamp": str(frame["timestamp"].iloc[-1]),
            "close": _last(c),
            "open": _last(o),
            "ema20": _last(e20),
            "ema50": _last(e50),
    ...
    volatility_72h = adaptive_volatility_profile(
        frame30["high"],
        frame30["low"],
        frame30["close"],
        baseline_bars=int(
            getattr(settings, "volatility_baseline_bars_30m", 144)
        ),
        recent_bars=int(getattr(settings, "volatility_recent_bars_30m", 12)),
        shock_bars=int(getattr(settings, "volatility_shock_bars_30m", 4)),
        expansion_ratio=float(
            getattr(settings, "volatility_expansion_ratio", 1.50)
        ),
        expansion_min_bars=int(
            getattr(settings, "volatility_expansion_min_bars", 3)
        ),
    )
    scan_standards = adaptive_scan_standards(
        volatility_72h,
        base_ranking_score=float(getattr(settings, "ranking_min_score", 55)),
        trend_score_relief=float(
            getattr(settings, "adaptive_trend_score_relief", 3)
        ),
    )
    breakout = breakout_state(
        ...
        turnover_ratio,
        f30["adx"],
        atr_multiple=float(scan_standards["breakout_atr_multiple"]),
        min_turnover_ratio=float(scan_standards["min_turnover_ratio"]),
        min_adx=float(scan_standards["min_adx"]),
    )
    ...
    return {
        ...
        "features": {
            ...
            "scan_standards": scan_standards,
            "ticker": ticker_24h,
            "coinglass": collected.get("coinglass", {}),
            "data_integrity": integrity,
            "market_session": collected.get("market_session", {}),
        },
        "market_state": regime,
