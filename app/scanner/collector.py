...
class MarketCollector:
    def __init__(self, client: Any, settings: Any, coinglass: Any | None = None):
        self.client = client
    ...
    async def collect_batch(self, universe: list[dict[str, Any]], as_of=None) -> list[dict[str, Any]]:
        semaphore = asyncio.Semaphore(
            int(getattr(self.settings, "bitget_max_concurrency", getattr(self.settings, "gate_max_concurrency", 8)))
        )
