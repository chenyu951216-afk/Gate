...
def rank_analysis(analysis: dict[str, Any], settings: Any, top_n: int = 10) -> dict[str, Any] | None:
    ...
    coinglass = features.get("coinglass", {})
    liquidation = coinglass.get("liquidation", {}) if isinstance(coinglass, dict) else {}
    coinglass_bias = float(liquidation.get("directional_bias") or 0.0)
    direction_sign = 1.0 if direction == "long" else -1.0
    coinglass_adjustment = (
        max(-4.0, min(4.0, coinglass_bias * direction_sign * 4.0))
        if coinglass.get("available")
        else 0.0
    )
    score = max(
        0.0,
    ...
    qualifies = (
        ...
        and (
            not getattr(settings, "coinglass_enabled", False)
            or not getattr(settings, "coinglass_required_for_ranking", True)
            or bool(coinglass.get("available"))
        )
    )
    return {
        ...
        "coinglass_adjustment": coinglass_adjustment,
        "direction_edge": edge,
