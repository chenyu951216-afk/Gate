...
- CoinGlass v4 清算歷史與清算熱圖資料；清算密集區可作為 TP 候選，若 API/方案暫時不可用會保留診斷並依設定阻止不完整排名進入實盤。
...
CoinGlass 使用 `CG-API-KEY` 認證。設定 `COINGLASS_ENABLED=true` 與 `COINGLASS_API_KEY` 後，掃描會取 30m 聚合清算歷史，並優先嘗試清算熱圖；`COINGLASS_REQUIRE_HEATMAP=true` 只適合已購買支援熱圖 endpoint 的方案，否則維持 `false` 使用清算歷史備援。
...
`COINGLASS_API_KEY=REPLACE_WITH_COINGLASS_API_KEY`

`COINGLASS_BASE_URL=https://open-api-v4.coinglass.com`

`COINGLASS_INTERVAL=30m`

`COINGLASS_USE_HEATMAP=true`

`COINGLASS_HEATMAP_RANGE=1d`

`COINGLASS_REQUIRE_HEATMAP=false`

`COINGLASS_REQUIRED_FOR_RANKING=true`

`COINGLASS_MAX_SYMBOLS_PER_SCAN=100`
