...
def _volatility_ohlc(ranges: list[float]) -> tuple[pd.Series, pd.Series, pd.Series]:
    close = pd.Series(np.full(len(ranges), 100.0))
    half = pd.Series(ranges) / 2
    return close + half, close - half, close


def test_volatility_profile_detects_persistent_two_hour_expansion():
    high, low, close = _volatility_ohlc([0.5] * 140 + [2.0] * 4)
    profile = adaptive_volatility_profile(high, low, close)
    assert profile["regime"] == "expansion"
    assert profile["confirmed_expansion"] is True
    assert profile["effective_atr_pct"] > profile["baseline_q95_pct"]


def test_volatility_profile_separates_one_liquidation_wick():
    high, low, close = _volatility_ohlc([0.5] * 143 + [8.0])
    profile = adaptive_volatility_profile(high, low, close)
    assert profile["regime"] == "isolated_spike"
    assert profile["isolated_spike"] is True
    assert profile["effective_atr_pct"] < profile["latest_true_range_pct"] / 4
