...
def test_normal_mode_rejects_ant_position_when_capacity_is_too_small():
    settings = Settings(_env_file=None)
    with pytest.raises(TradingRiskError) as error:
        dynamic_position_notional(
            equity=10_000, available_margin=20, entry=100, stop=98,
            metrics=_candidate()["metrics"], side="long", stop_quality="STRUCTURE",
            market_state="normal", risk_flags=[], settings=settings, leverage=2,
        )
    assert error.value.code == "POSITION_BELOW_VIABLE_SIZE"
