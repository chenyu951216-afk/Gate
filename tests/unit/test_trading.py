...
def test_zec_uses_twenty_thousand_usdt_notional_without_becoming_a_driver_slot():
    settings = Settings(_env_file=None)
    service = TradingService(SimpleNamespace(rest=None), FakeRepository(), settings)
    assert service._notional("ZEC_USDT") == 20_000
...
class FakeRest:
    ...
    async def place_futures_order(self, body):
        ...
        if body.get("reduce_only"):
            return {"id": "emergency"}
        self.positions = [{"contract": "BTC_USDT", "size": "4000", "entry_price": "100000", "lever": "100"}]
