...
from app.trading.risk import build_execution_plan, max_leverage_for_notional, notional_for_contract
...
class FakeRest:
    ...
    async def place_futures_order(self, body):
        ...
        if body.get("reduce_only"):
            return {"id": "emergency"}
        self.positions = [{"contract": "BTC_USDT", "size": "4", "entry_price": "100000", "lever": "100"}]
...
async def test_take_profit_extension_moves_only_in_favour_and_creates_replacement():
    ...
    assert rest.protection[-1]["trigger"]["price"] == str(plan["take_profits"][2]["price"])
