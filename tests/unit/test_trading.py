...
from app.trading.position_thesis import assess_closed_15m_thesis
...
def test_managed_stop_waits_for_proof_then_covers_fees():
    ...
    early = managed_stop_candidate(
        plan=plan,
        context={"atr15": 1, "last_close15": 102},
        price=102,
        entry=100,
        settings=settings,
    )
    ...
    confirmed = managed_stop_candidate(
        plan=plan,
        context={"atr15": 1, "last_close15": 102.4},
        price=102.4,
        entry=100,
        settings=settings,
    )
...
@pytest.mark.asyncio
async def test_time_decay_arms_5m_exit_instead_of_clock_only_close():
    ...
    assert result["position_thesis_state"] == "EXIT_ARMED"
    assert closed == []


@pytest.mark.asyncio
async def test_confirmed_15m_loss_uses_5m_continuation_to_execute_exit():
    settings = Settings(_env_file=None, position_management_enabled=True)
    rest = FakeRest()
    repo = FakeRepository()
    service = TradingService(SimpleNamespace(rest=rest), repo, settings)
    plan = build_execution_plan(_candidate(), _info(), settings, 100000)
    record = service._managed_payload(
        "BTC_USDT:long", plan, 4000, {}, 20
    )
    closed = []
    ...
    service._ensure_protection = no_missing
    context = {
        "closed_timestamp15": "2026-07-27T12:45:00+00:00",
        "last_close15": 98000,
        "last_high15": 99000,
        "last_low15": 97500,
        "atr15": 1000,
    ...
    result = await service._manage_position(
        ...
        {"mark_price": "99000", "last": "99000"},
        context,
        _info(),
    )
    assert result["status"] == "position_exit_executed"
    assert result["reason"] == "decisive_5m_adverse_continuation"
...
async def test_opposite_signal_must_persist_before_position_reversal():
    ...
    repo.positions["BTC_USDT:short"] = {
        ...
        "plan": {
            "contract": "BTC_USDT",
            "side": "short",
            "position_thesis_state": "EXIT_ARMED",
        },
    }
    service = TradingService(SimpleNamespace(rest=rest), repo, settings)
...
def test_5m_noise_cannot_create_a_15m_thesis_failure():
    settings = Settings(_env_file=None)
    context = {
        "closed_timestamp15": "2026-07-27T12:30:00+00:00",
        "last_close15": 99,
        "atr15": 2,
        "ema2015": 100,
        "ema5015": 98,
        ...
        "mfi15": 48,
        "turnover_ratio15": 1,
        "ema20_slope_atr15": -0.02,
        "below_trend_count15": 1,
        "last_close5": 90,
        "ema205": 95,
    }
    assessment = assess_closed_15m_thesis("long", context, settings)
    assert assessment["candidate"] is False
    assert all(not item.startswith("5m_") for item in assessment["evidence"])


def test_decisive_closed_15m_reversal_can_confirm_without_a_time_lock():
    settings = Settings(_env_file=None)
    plan = {"side": "long"}
    context = {
        "closed_timestamp15": "2026-07-27T12:30:00+00:00",
...
def test_soft_reversal_counts_completed_15m_candles_not_manager_ticks():
    settings = Settings(_env_file=None)
    plan = {"side": "long"}
    context = {
        "closed_timestamp15": "2026-07-27T12:30:00+00:00",
        ...
        "mfi15": 45,
        "turnover_ratio15": 1,
        "ema20_slope_atr15": -0.2,
        "below_trend_count15": 2,
    }
    assessment = assess_closed_15m_thesis("long", context, settings)
    ...
    context["closed_timestamp15"] = "2026-07-27T12:45:00+00:00"
    second_assessment = assess_closed_15m_thesis(
    ...
    assert assessment["decisive"] is False
