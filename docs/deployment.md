...
限價單使用接近買一／賣一的價格，最多等待 10,800 秒（3 小時）；一般動能撤單至少觀察 30 秒並連續確認，門檻至少 1.5%，且會依共用自適應波動尺度放寬至最多 5%。單向位移達 3% 以上才會進入至少 10 秒的快速撤單。掛單成交後由持倉管理接管並補掛完整 Bitget 保護單，撤單前後都會回讀確認並通知原因。Zeabur 新增環境變數可直接由 `.env.example` 複製；其中 `THESIS_SOFT_FAILURE_CONFIRMATIONS=2` 與 `REVERSAL_SIGNAL_CONFIRMATIONS=2` 都是跨已收 30m K 的證據確認，不是最低持倉時間。
