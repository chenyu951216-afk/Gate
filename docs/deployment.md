...
6. 要啟用實盤，明確設定 `AUTO_ORDER_ENABLED=true` 與 `POSITION_MANAGEMENT_ENABLED=true`，並填入 `GATE_API_KEY`、`GATE_API_SECRET`、`SCAN_DISCORD_WEBHOOK_URL`、`ORDER_DISCORD_WEBHOOK_URL`。兩個 webhook 不要填同一個群組；`MINIMUM_ORDER_RR=1.0`、`MAX_INITIAL_STOP_LOSS_USDT=1000`。
