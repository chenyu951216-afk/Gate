...
交易流程：合格排名 → 查 Gate 實際持倉與 open orders → 主要幣 2 種上限／同幣防重／名目金額 → risk-limit tier 最高槓桿 → cross 限價單（進場單先綁初始止損）→ 回讀確認交易所止損與 TP1/TP2/TP3 → PostgreSQL 管理狀態。持倉管理是獨立 5 秒循環，不等待 30m 掃描；交易所保護優先，只有交易所保護失敗且觸價時才送後台 reduce-only 備援；pause 只阻止新建倉，不停止既有保護。
