...
- 限價單一直未成交：後台每 5 秒持續讀取 Bitget 掛單與持倉；查看下單通知的 `entry_limit_price` 與 `entry_order_id`。一般動能撤單要滿足至少 30 秒觀察、連續 2 次確認，門檻至少 1.5%，並會依該幣 72h／6h／2h 共用波動尺度放寬至最多 5%；3% 以上單向急走才使用至少 10 秒的快速撤單。否則最久等待 `LIMIT_ENTRY_TIMEOUT_SECONDS=10800`（3 小時）。
- 方向反轉未換倉：完整的反向排名要在兩根不同的已收 30m K 持續成立，才會先撤 Bitget 保護單與舊限價單、確認舊持倉為零，再送新方向限價單。第一根會通知 `reversal_confirmation_pending`；真正的 30m 結構失效仍可由持倉管理立即退出。若撤單或平倉未獲交易所確認，系統會拒絕反手，避免新舊方向同時存在。
- 掃描後保護點位未更新：30m 掃描只更新最新論點失效位、目標與波動制度供稽核，不會和 5 秒持倉管理同時搶改保護單。實際移動止損只能單向收緊，每次至少移動 `0.20` 個共用波動尺度，且替換時會先確認新單存在，再撤舊單並確認舊單消失。TP1／TP2／TP3 不向遠處追價；只有已獲利至少 1R 且 30m／15m 同時轉弱時，下一個 TP 才可縮近。
- 正式模式掃描但沒有下單：先看 `/api/scan/latest` 的 `trading.orders`。若 CoinGlass 沒有可用熱圖或目標價不滿足 RR，系統會回退到本策略的 R 倍數 TP；TP1 仍必須達到 `MINIMUM_ORDER_RR=1.20`，並不會因正式模式而放寬。
