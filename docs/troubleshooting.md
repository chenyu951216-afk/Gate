...
- 限價單一直未成交：後台每 5 秒持續讀取 Bitget 掛單與持倉；查看下單通知的 `entry_limit_price` 與 `entry_order_id`。一般動能撤單要滿足至少 30 秒觀察、連續 2 次確認，門檻至少 1.5%，並會依該幣 15m ATR 放寬至最多 5%；3% 以上單向急走才使用至少 10 秒的快速撤單。否則最久等待 `LIMIT_ENTRY_TIMEOUT_SECONDS=10800`（3 小時）。
- 方向反轉未換倉：同一幣種的新方向訊號會先撤 Bitget 保護單與舊限價單，確認舊持倉為零後才送出新方向限價單；若撤單或平倉未獲交易所確認，系統會拒絕反手，避免新舊方向同時存在。
- 掃描後保護點位未更新：只有同方向、仍在持倉中的候選才會同步；止損至少移動 `0.15 ATR`（且不會放寬），止盈至少移動 `0.2 ATR`，或在已達 1R 後依最新結構縮近以保護利潤。替換時會先確認新單存在，再撤舊單並確認舊單消失。
