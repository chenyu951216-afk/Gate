...
- 方向反轉未換倉：完整反向排名除了要跨兩根不同的已收 30m K，持倉專屬 15m 論點也必須進入 `EXIT_ARMED`。第一根或 15m 尚未確認時會通知 `reversal_confirmation_pending`；這是避免掃幣排名與持倉監控互相搶單。若撤單或平倉未獲交易所確認，系統仍拒絕反手。
- 掃描後保護點位未更新：30m 掃描只更新最新論點失效位、目標與波動制度供稽核，不會和持倉管理器搶改保護單。實際移動止損只能單向收緊，而且保本啟動採已收 15m close 的峰值，不採每 5 秒 mark high；`WATCH`／`EXIT_ARMED` 期間也不會突然把止損勒緊。替換時先確認新單存在，再撤舊單並確認消失。
